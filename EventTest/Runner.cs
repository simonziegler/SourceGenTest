...
namespace EventTest
{
    internal class Runner
    {
        ...
        public Runner()
        {
            (_code, _generator) = LoadViewModelNotifier();
            s_references = new();
            s_references.Add(MetadataReference.CreateFromFile(typeof(object).Assembly.Location));
            s_references.Add(MetadataReference.CreateFromFile(typeof(System.Text.StringBuilder).Assembly.Location));
            s_references.Add(MetadataReference.CreateFromFile(typeof(Microsoft.CodeAnalysis.CSharp.Conversion).Assembly.Location));
        }

        internal void Run(string baseUri)
        {
            if (s_references == null)
            {
                s_references = GetReferences(baseUri);
            }
        ...
        private static List<MetadataReference> GetReferences(string baseUri)
        {
            ...
            {
                Stream? stream = client.GetStreamAsync($"_framework/_bin/{reference.Location}").Result;
        ...
        private static (string, string) LoadViewModelNotifier()
        {
            using var streamReader1 = new StreamReader(typeof(Runner).Assembly.GetManifestResourceStream("EventTest.Scheme.cs")!);
            using var streamReader2 = new StreamReader(typeof(Runner).Assembly.GetManifestResourceStream("EventTest.ViewModelNotifierGenerator.cs")!);
            //using var streamReader2 = new StreamReader(typeof(ViewModelNotifierGenerator).Assembly.GetManifestResourceStream("ViewModelNotifierGenerator.cs")!);

            return (streamReader1.ReadToEnd(), streamReader2.ReadToEnd());
        }
    }
}
